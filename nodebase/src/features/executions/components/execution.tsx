...
import { CheckCircle2Icon, Loader2Icon, XCircleIcon, ClockIcon, Link } from "lucide-react";
...
export const ExecutionView = ({executionId}: {executionId: string}) => {
    ...
    const duration = execution.finishedAt ? Math.round(
        (new Date(execution.finishedAt).getTime() - new Date(execution.startedAt).getTime()) / 1000
    ) : null;

    return (
        <Card className="shadow-none">
            <CardHeader className="flex items-centre gap-3">
                <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    {execution.finishedAt && (
                        <div>
                            ...
                            <p className="text-sm">{formatDistanceToNow(execution.finishedAt, {addSuffix: true})}</p>
                        </div>
                    )}
                    ...
                    {execution.error && (
                        <div className="mt-6 p-4 bg-red-50 rounded-md space-y-3">
                            <div>
                                ...
                                <p className="text-sm text-red-800 text-mono">{execution.error}</p>
                            </div>
                    ...
                    {execution.output && (
                        <div className="mt-6 p-4 bg-green-50 rounded-md space-y-3">
                            <div>
                                ...
                                <p className="text-sm text-mono">{JSON.stringify(execution.output, null, 2)}</p>
                            </div>
                        </div>
                    )}
            </CardContent>
